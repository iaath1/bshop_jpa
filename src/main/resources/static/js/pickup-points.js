...
function displayPoints() {
    ...
    container.innerHTML = pointsToShow.map(point => {
        ...
        return `
        <div class="pickup-point${isSelected ? ' selected' : ''}"
             data-point-id="${escapeAttr(point.id)}"
             onclick='selectPoint(${JSON.stringify(point).replace(/'/g,"&#39;")})'>

            <div class="pickup-point-inner">
                <div class="pickup-point-check">✓</div>
                <div class="pickup-point-content">
                    <strong>${escapeHtml(point.name)}</strong>
                    <span class="pickup-point-address">${escapeHtml(point.address)}</span>
                    <small class="pickup-point-meta">
                        ${extraInfo.join(' | ')} | 
...
window.selectPoint = function(point) {
    ...
    // Find and highlight the clicked point by data-point-id
    document.querySelectorAll('.pickup-point').forEach(el => {
        if (el.getAttribute('data-point-id') === String(point.id)) {
